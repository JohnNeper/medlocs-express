...
import { Bell, MapPin, Search, Plus, FileText, ChevronRight, Sparkles } from "lucide-react";
import logo from "@/assets/logo.png";
import { POPULAR, PROMOS } from "@/lib/medlocs-data";
...
function HomePage() {
  ...
  const [filter, setFilter] = useState<"open" | "duty" | "near" | null>(null);
    ...
    <AppShell>
      ...
      <div className="px-5">
        ...
        >
          ...
          <Link
            to="/ordonnance"
            className="grid place-items-center h-9 w-9 rounded-xl bg-primary-soft text-primary"
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 pb-1">
          {[
            ...
            return (
              <button
                key={f.id}
                onClick={() => setFilter(active ? null : (f.id as any))}
        ...
        <div className="mt-5">
          <MapCard onClick={() => goSearch(query || "Paracétamol 500mg")} />
        </div>
